# 🏗️ Ingenium - System Architecture
...
└─────────────────┘  └─────────────────┘   └──────────────────┘
         ...
         ▼                    ▼                       ▼
┌─────────────────┐  ┌─────────────────┐   ┌──────────────────┐
│ USER DASHBOARD  │  │    HOSPITAL     │   │   GOVERNMENT     │
│                 │  │    DASHBOARD    │   │    DASHBOARD     │
│ ┌─────────────┐ │  │                 │   │                  │
│ │  🩺 MEDICAL │ │  │ View Patients   │   │ Review Farmer    │
│ │  SERVICES   │ │  │ Access Data     │   │  Applications    │
│ └─────────────┘ │  │ Upload Records  │   │                  │
│                 │  │ Create Bills    │   │ Update Status    │
│ ┌─────────────┐ │  └─────────────────┘   │ Add Replies      │
│ │  🌾 FARMER  │ │                         └──────────────────┘
│ │  SERVICES   │ │
│ └─────────────┘ │
...
│  2. View active patients (GET /hospital/patients)   │
│  3. Select patient                                   │
│  4. View details (GET /hospital/patient/:id)        │
│     • System checks access permissions              │
│     • Filters data based on scope                   │
│     • Creates audit log entry                       │
│  5. Upload records (if permitted)                   │
│     • POST /hospital/records/upload                 │
│  6. Create bills                                     │
│     • POST /hospital/bills/create                   │
└─────────────────────────────────────────────────────┘
                         │
                         │
                         ▼
...
## 🌾 Farmer Module Flow (SECONDARY MODULE)

```
USER DASHBOARD
      │
      │ Select "Farmer Services"
      ▼
┌──────────────────────┐
│ Check Farmer         │
│ Account Status       │
...
│              AUTHORIZATION LAYER                     │
│                                                      │
│  1. Check user role/type                            │
│  2. Verify route permissions                        │
│  3. Check resource ownership                        │
│  4. Validate access scope (for hospitals)           │
└─────────────────────────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────┐
...
│                  AUDIT LOGGING                       │
│                                                      │
│  Record all access events (immutable)               │
└─────────────────────────────────────────────────────┘
```
